					...
					{todos.map((todo, index) => (
						...
						>
							{editableTodoIndex === index ? (
								<p className="w-[150px] px-2 py-1 rounded-md bg-slate-600 text-white">{yangiTodo}</p>
							) : (
								<span>{todo}</span>
							)}
...
//     newTodo: Manages the input value for adding or editing todos.
//     todos: Keeps track of the list of todos.
//     editableTodoIndex: Tracks the index of the todo being edited.


//     handleInputChange: Updates the newTodo state as the user types.
//     handleAddTodo: Adds a new todo to the list.
//     handleDeleteTodo: Deletes a todo from the list.
//     handleEditTodo: Enables editing mode for a specific todo.
//     handleSaveEdit: Saves the changes made during editing.
//     handleCancelEdit: Cancels the editing mode.
//     handleKeyPress: Handles the Enter key press for adding or saving todos.